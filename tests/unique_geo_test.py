...
def test_can_import_compressed_file():
    ...
    assert sorted(imported_file['lat']) == sorted([41.37484808,
                                                   41.37438124,
                                                   41.3806658,
                                                   41.389865])
    assert sorted(imported_file['lng']) == sorted([2.11886538,
                                                   2.11842003,
                                                   2.1647467,
                                                   2.1556433])
    assert sorted(imported_file['geohash']) == sorted(['sp3e2hncvnsw3r1h',
                                                       'sp3e25yx4z8mwv4r',
                                                       'sp3e3m07f70ns0',
                                                       'sp3e3nc9qnh1u'])
...
def test_can_import_non_compressed_file():
    ugh = UniqueGeoHash()
    imported_file = ugh.import_non_compressed_file(
...
def test_ugh_prefix_list_is_same_length():
    ...
    assert len(imported_file['geohash']) == len(ugh_prefixs)
